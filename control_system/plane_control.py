...
def limit_command(command, limit_min, limit_max):

    if command < limit_min:
        command = limit_min
    elif command > limit_max:
        command = limit_max
...
class LongitudinalAutoPilot(object):
    def __init__(self):
        ...
        self.max_pitch_cmd = 10.0*np.pi/180.0       # Default was 30 deg
    ...
    def pitch_loop(self, pitch, pitch_rate, pitch_cmd):
        ...
        # Errors
        e = pitch_cmd - pitch
        e_d = pitch_rate # Assuming derivative of error equal to pitch_rate q        

        # PD controller
        elevator_cmd = k_p_theta*e + k_d_theta*e_d

        # Command limitations
        elevator_cmd = limit_command(elevator_cmd, -1, 1)   

        # Uncomment to disable controller.
        # elevator_cmd = 0.0
    ...
    def altitude_loop(self, altitude, altitude_cmd, dt):
        ...
        # Errors
        e = altitude_cmd - altitude
        self.alt_int += e*dt

        # PI Controller
        pitch_cmd = k_p_z*e + k_i_z*self.alt_int

        # Command limitations
        pitch_cmd = limit_command(pitch_cmd, -self.max_pitch_cmd, self.max_pitch_cmd)
        
        # END STUDENT CODE

        # Uncomment disable controller.
        # pitch_cmd = 0.0
